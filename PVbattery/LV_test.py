...
# Extract the price lists from the second column ("price")
purchase_prices = df_purchase["price"].tolist()
sale_prices = df_sale["price"].tolist()

# 初始化总收入与总购电成本（单位：元或其他货币单位，取决于price文件的单位）
total_revenue = 0.0
total_purchase_cost = 0.0
electricity_cost_over_time = []
battery_soc = {conf['bess_name']: 20.0 for conf in pv_bess_config_lv}  # 初始化SOC=50%
...
for step_idx in range(total_number):
    ...
    for config in pv_bess_config_lv:
        ...
        # 从先前读入的DataFrame中获取当前时刻的PV及Load数据
        pv_power_kW = config["pv_data"].iloc[step_idx]["pv"]
        load_kW = config["load_data"].iloc[step_idx]["load"]
        net_power = pv_power_kW - load_kW  # 正表示多余功率（可用于充电或卖电），负表示短缺（需放电或购电）
        ...
        energy_charged = 0.0  # 本时段用于充电的能量（单位：kWh）
        energy_discharged = 0.0  # 本时段电池放电的能量（单位：kWh）
        ...
        if net_power > 0:
            ...
            if surplus_energy > 0:
                print(
                    f"    -> Surplus energy: {surplus_energy:.2f} kWh sold at price {sale_prices[step_idx]:.4f}, revenue += {revenue:.4f}")
            ...
            if current_soc > 20.0:
                ...
                # 不足部分需要从电网购电： deficit = 本时段总缺口能量 - 放电补充能量
            ...
            if shortage_energy > 0:
                print(
                    f"    -> Shortage energy: {shortage_energy:.2f} kWh bought at price {purchase_prices[step_idx]:.4f}, cost += {cost:.4f}")
        else:
            # net_power==0，不存在交易
            dss.text(f"Edit Storage.{bess_name} State=IDLING kW=0")
    ...
    dss.text("Solve")
    # 打印当前累计净成本（单位：£）
    current_electricity_cost = (total_purchase_cost - total_revenue)  # 已在计算时转换单位
    electricity_cost_over_time.append(current_electricity_cost)
    print(f"Time step {step_idx + 1}: Electricity cost = £{current_electricity_cost:.2f}")  # 添加£符号
...
print(f"Total revenue from surplus energy: £{total_revenue:.2f}")       # 添加£符号
print(f"Total purchase cost for shortage energy: £{total_purchase_cost:.2f}")  # 添加£符号
print(f"Net profit: £{total_revenue - total_purchase_cost:.2f}")        # 添加£符号
print("=============================================\n")
...
phase2_dict = {}
phase3_dict = {}
user_bus_dict = {}  # 存储 UserX.1 母线电压

# 改用 strip() 去除潜在空格，并且用 lower() 实现大小写不敏感比较
for node_name, v_list in node_voltages_over_time.items():
    ...
    if clean_name.endswith(".1"):
        if clean_name.lower().startswith("user"):
            # 将所有以"user"开头的母线分离到 user_bus_dict
