...
def add_components_to_dss():
    for config in pv_bess_config:
        ...
        dss.text(
            f"New Loadshape.shape{config['load_name']} npts=48 minterval=30 mult=(file={loadshape_file}) useactual=no"
        )

        dss.text(
            f"New Load.{config['load_name']} Phases=1 Bus1={config['bus_name']} kV={config['kV']} kW={config['load_kW']} PF={config['pf']} daily=shape{config['load_name']}"
        )
    ...
    for config in pv_bess_config:
        ...
        if net_power > 0:
            if battery_soc[bess_name] < 80.0:
                ...
                Original_SOC = dss.text(f"? storage.{bess_name}.%stored")
                print(f"dssOriginal SOC: {Original_SOC}")

                dss.text(f"Edit Storage.{bess_name} State=CHARGING kW={charge_power} %stored={battery_soc[bess_name]}")
                new_SOC = dss.text(f"? storage.{bess_name}.%stored")


                print(f"dssNew SOC: {new_SOC}")
...
dss.text("Export monitors all")
        ...
        # Solve the circuit for the current configuration
        #dss.text("Solve")
