    ...
    for step_idx in range(total_number):
        ...
        for i, config in enumerate(pv_bess_config_lv):
            ...
            if original:
                ...
                # 设定电池的SOC限制
                min_soc = 20.0  # 最低允许SOC（%）
                max_soc = 80.0  # 最高允许SOC（%）

                # ---- PSO 优化调度部分的修改 ----

                # 获取当前时刻的 PSO 调度结果
                battery_power = battery_powers[i, step_idx]  # 单位 kW；正值：放电，负值：充电



                # 根据PSO结果执行充放电操作（不再与net_power挂钩）
                battery_power = battery_powers[i, step_idx]  # kW. Positive for discharge, negative for charge

                if battery_power > 0:  # 放电
                    # 放电能量受限于PSO设定值和剩余可放电能量
                    energy_discharged = min(battery_power * time_step_hours,
                                            ((current_soc - min_soc) / 100.0) * battery_capacity_kWh)
                    soc_decr = (energy_discharged / battery_capacity_kWh) * 100
                    new_soc = current_soc - soc_decr
                    battery_soc[bess_name] = max(new_soc, min_soc)  # 确保不低于最低SOC

                elif battery_power < 0:  # 充电
                    # 充电能量受限于PSO设定值和剩余可充能量
                    energy_charged = min(abs(battery_power) * time_step_hours,
                                         ((max_soc - current_soc) / 100.0) * battery_capacity_kWh)
                    soc_incr = (energy_charged / battery_capacity_kWh) * 100
                    new_soc = current_soc + soc_incr
                    battery_soc[bess_name] = min(new_soc, max_soc)  # 确保不超过最高SOC
                # 当 battery_power == 0 时，不进行充放电操作

                # 计算经过电池调度后，与电网之间的能量交换（单位：kWh）
                # 注：net_energy = (PV - 负载) * time_step_hours
                net_energy = net_power * time_step_hours
                if battery_power > 0:
                    # 放电时，电池输出能量，增加了上网的能量
                    effective_grid_energy = net_energy + energy_discharged
                elif battery_power < 0:
                    # 充电时，电池吸收能量，减少了上网的能量
                    effective_grid_energy = net_energy - energy_charged
                else:
                    effective_grid_energy = net_energy

                # 根据调整后能量计算成本或收入
                if effective_grid_energy > 0:
                    # 上网售电：收入为正，但在成本计算中记作负值
                    revenue = (effective_grid_energy * sale_prices[step_idx]) / 100
                    total_revenue += revenue
                    battery_cost_inc = -revenue
                elif effective_grid_energy < 0:
                    # 需从电网购电：产生成本
                    shortage_energy = abs(effective_grid_energy)
                    cost = (shortage_energy * purchase_prices[step_idx]) / 100
                    total_purchase_cost += cost
                    battery_cost_inc = cost
                else:
                    battery_cost_inc = 0.0

                # 累计该时间步内该家庭（或配置）的电费成本
...
# 调整后的PSO参数设置
# 设定搜索空间维度和边界
dim = len(pv_bess_config_lv) * total_number
lower_bound = -3 * np.ones(dim)
upper_bound = 3 * np.ones(dim)

# PSO参数设置
n_particles = 300       # 增加粒子数量
iterations = 300        # 增加迭代次数
options = {
    'c1': 2.5,         # 个体加速常数
    'c2': 2.5,         # 社会加速常数
    'w': 0.9           # 惯性权重（初始值，可以考虑动态衰减）
}
